...
def login(request):
    ...
    if "idp" in req["get_data"]:
        ...
        print("------ REQUEST session login (before) ------")
        print(request.session.session_key)
        print(str(request.session.items()))
        request.session["idp"] = idp
        request.session.save()
        print("------ REQUEST session login (after) ------")
        print(request.session.session_key)
        print(str(request.session.items()))
...
def attributes_consumer(request):
    ...
    """
    req = prepare_django_request(request)
    print("------ REQUEST session  attribute-consumer ------")
    print(request.session.session_key)
    print(str(request.session.items()))
    ...
    if idp:
        ...
        if not errors:
            ...
            return HttpResponseRedirect(redirect_to)
    print("NO IDP in session! Returning HTTP ResponseError")
    return HttpResponseServerError()
